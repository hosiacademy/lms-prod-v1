"""
Management command to create the PRODUCTION $5 USD Learnership Programme.
This is the ACTUAL production learnership with real go-live details.
Marked as featured (RED card emphasis) for easy identification.

Usage:
    python manage.py create_production_five_dollar_learnership
"""
...
class Command(BaseCommand):
    help = "Create the PRODUCTION $5 USD Learnership Programme"

    def handle(self, *args, **kwargs):
        self.stdout.write("Creating PRODUCTION $5 USD Learnership Programme...")
        ...
        # Create the $5 PRODUCTION Learnership Programme
        title = 'AI+ Digital Literacy Learnership - $5'
        slug = slugify(title)

        # Check if already exists
        if LearnershipProgramme.objects.filter(slug=slug).exists():
            self.stdout.write(self.style.WARNING(
                f"Production Learnership already exists with slug {slug}, skipping..."
            ))
            return
        ...
        learnership = LearnershipProgramme(
            ...
            description='Join HOSI Academy transformative AI+ Digital Literacy Learnership! '
                       ...
                       'across Africa. This special $5 learnership is part of HOSI Academy mission '
                       'to make AI education accessible to all Africans.',
        ...
        self.stdout.write(self.style.SUCCESS(
            f"✓ Created PRODUCTION Learnership: {learnership.title}"
        ))
        self.stdout.write(self.style.SUCCESS(
        ...
        self.stdout.write(self.style.SUCCESS(
            f"\n✅ Successfully created PRODUCTION $5 Learnership Programme!"
        ))
        self.stdout.write(self.style.SUCCESS(
