...
import 'package:frontend/src/presentation/pages/onboarding/widgets/sections/safe_network_image.dart';
...
class MasterclassCalendarState extends State<MasterclassCalendar> {
  ...
  int _selectedMonthIndex = 1; // 1-12 for specific months
  late DateTime _baseDate;
  int? _highlightedQuarter;
  List<int> _highlightedMonthIndices = [];
  bool _isAllMode = true; // NEW: Default to showing all masterclasses
  ...
  Map<DateTime, List<Masterclass>> get _monthEvents {
    ...
    widget.events.forEach((date, masterclasses) {
      ...
      if (_isAllMode) {
        ...
        final actualMonthIndex = (_availableMonths.indexWhere(
                (m) => m.year == date.year && m.month == date.month) +
            1);

        if (actualMonthIndex > 0) {
          matchesFilter = _highlightedMonthIndices.isNotEmpty
  ...
  void _highlightQuarterMonths(int quarter, List<int> monthIndices) {
    setState(() {
      _isAllMode = false; // Disable ALL mode when a quarter is clicked
      _highlightedQuarter = quarter;
      _highlightedMonthIndices = monthIndices;
      if (monthIndices.isNotEmpty) {
        _selectedMonthIndex = monthIndices.first;
      }
    });
  }
          ...
          child: Row(
            ...
            children: [
              ...List.generate(12, (index) {
                ...
                final isSelected =
                    !_isAllMode && actualIndex == _selectedMonthIndex;
                final quarter = _getQuarterForDate(monthDate);
                final isHighlighted = !_isAllMode &&
    ...
    return Expanded(
      child: Container(
        ...
        child: GestureDetector(
          onTap: () {
            setState(() {
              _isAllMode = false; // Disable ALL mode when a month is clicked
  ...
  List<Widget> _buildQuarterTabs() {
    ...
    for (final key in sortedKeys) {
      ...
      final monthIndices = monthsInQuarter.map((m) {
        return _availableMonths
                .indexWhere((am) => am.year == m.year && am.month == m.month) +
            1;
      }).toList();
        ...
        Expanded(
          ...
          child: Container(
            ...
            child: _QuarterTab(
              ...
              onTap: () => _highlightQuarterMonths(quarter, monthIndices),
            ),
          ),
        ),
      );
          ...
          children: [
            ...
            Text(
              _isAllMode
                  ...
                  : 'No masterclasses scheduled for ${_getFullMonthName(_selectedMonth)} ${_selectedMonth.year}',
...
class _MasterclassHoverCardState extends State<_MasterclassHoverCard> {
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: colorScheme.surface, // Responsive background
            borderRadius: BorderRadius.circular(16),
            boxShadow: [
              BoxShadow(
            ...
            border: Border.all(
              color:
                  widget.streamColor.withValues(alpha: _isHovered ? 0.5 : 0.2),
                              ...
                              return Container(
                                ...
                                child: ClipOval(
                                  child: widget.certificationImageUrl != null
                                      ...
                                      : _buildInitialsCircle(masterclass.title),
                                ),
                              );
                            },
                          ),
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            masterclass.title,
                            style: theme.textTheme.labelMedium?.copyWith(
                              color: colorScheme.onSurface, // Responsive text
                              fontWeight: FontWeight.w800,
                              fontSize: 11,
                              height: 1.25,
                            ),
                  ...
                  child: IgnorePointer(
                    ...
                    child: Container(
                      color: colorScheme.surface
                          .withValues(alpha: 0.98), // Responsive overlay
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          // Header: title
                          ...
                          Expanded(
                            child: ListView.separated(
                              ...
                              itemBuilder: (context, i) {
                                ...
                                final flag = _countryFlag(session.countryName);
                                return Row(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Expanded(
  ...
  Widget _buildCertImage(String imageUrl) {
    // All AICerts images are SVG — use bytes-fetch proxy (same-origin, no CORS)
